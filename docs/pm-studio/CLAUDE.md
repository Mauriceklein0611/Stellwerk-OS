...
7. Doku aktualisieren: `docs/task-index.md` (Status), betroffene Docs, ggf. README. **Task-Archivierung im selben Branch/PR:** Sobald der Task fertig ist, den vollständigen Inhalt der `tasks/TASK-XXX.md` nach `docs/tasks-archive.md` verschieben (neueste zuerst), die Originaldatei in `tasks/` löschen und den Status im Index auf `done · archiviert` setzen – **alles im Task-Branch**, kein separater `chore`-Branch dafür.
...
- Die Routine ersetzt **nicht** den PR-Flow: Code- und Doku-Änderungen weiterhin
  nur über Feature-Branch + PR (auch `TODO.md`/`CLAUDE.md` selbst).
